<?php
	if(isset($_POST['cmdSave'])) {
		...
		$ssql = "DELETE FROM `tblpatient` WHERE `patient_id` = '".$doc_id."'";
		$data_result->executeSQL($ssql,"icare");
		
		$ssql = "DELETE FROM `tblusers` WHERE `user_id` = '".$doc_id."'";
		$data_result->executeSQL($ssql,"icare");
		...
		echo "<script>self.location = '?menu_id=0&sub_menu_id=3';</script>";
	}


	if(isset($_POST['cmdRemove'])) {
		$ssql = "DELETE FROM `tbldoctors` WHERE `doc_id` = '".$_POST['class_id']."'";
		$data_result->executeSQL($ssql,"icare");
	...
	<table id="example1" class="table table-bordered table-striped">
		<thead>
			<tr>
				...
				<th style="text-align:center;">Doctors ID</th>
		...
		<tbody>
			<?php
				$cc = 0;
				$ssql = "SELECT `doc_id`, `fullname`, `fname`, `mname`, `lname`, `specialist_id`, `photo_path`, `street`, `barangay`, `city_municipality`, `province`, `postal_code`, `contact_no`, `email_address` FROM `tbldoctors` ORDER BY `doc_id` ASC";
				...
				{ 
					...
					?>
						<tr>
							...
							<td><?php echo strtoupper($row['doc_id'])?></td>
							<td><?php echo strtoupper($row['fullname'])?></td>
							<td><?php echo strtoupper($row['specialist_id'])?></td>
							...
							<td>
								<button type="button" class="btn btn-xs btn-warning" data-toggle="modal" data-target="#modalEdit<?php echo strtoupper($row['doc_id'])?>"><small><b><i class="fa fa-pencil"></i> </b></small></button>
								<button type="button" class="btn btn-xs btn-danger" data-toggle="modal" data-target="#modalRemove<?php echo strtoupper($row['doc_id'])?>"><small><b><i class="fa fa-times"></i> </b></small></button>
							</td>
						</tr>
						<div class="modal fade" id="modalEdit<?php echo strtoupper($row['doc_id'])?>">
								...
								<div class="modal-content">
									...
									<form method="POST">
										<div class="modal-body">
											<div class="row">
												<div class="col-xs-12">
													<input type="hidden" class="form-control" name="class_id" required="true" value="<?php echo strtoupper($row['doc_id'])?>">
													<div class="col-xs-12" style="padding:2px;" id="doctorList">
														<small><b>Specialist of</b></small>
														<select class="form-control" name="specialist_id">
															<?php 
																$ssql = "SELECT `specialist_name` FROM `tblspecialist` WHERE specialist_name = '".$row['specialist_id']."' ORDER BY `specialist_name` ASC";
																$result_1 = $data_result->executeSQL($ssql,"icare");
																while($row_1 = mysqli_fetch_array($result_1))
																{
																	?>
																		<option value="<?php echo $row_1['specialist_name']?>"><?php echo strtoupper($row_1['specialist_name'])?></option>
																	<?php
																}
															?>
						...
						<div class="modal modal-danger fade" id="modalRemove<?php echo strtoupper($row['doc_id'])?>">
							<div class="modal-dialog">
								<div class="modal-content">
									<div class="modal-header">
										<button type="button" class="close" data-dismiss="modal" aria-label="Close">
