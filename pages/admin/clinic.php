<?php
	if(isset($_POST['cmdSave'])) {
		...
		echo "<script>self.location = '?menu_id=0&sub_menu_id=2';</script>";
	}
...
<div class="box-body">
	<table id="example1" class="table table-bordered table-striped">
		...
		<tbody>
			<?php
				$cc = 0;
				$ssql = "SELECT `clinic_id`, `clinic_name`, `contact_no`, `email_address`, `street`, `barangay`, `city_municipality`, `province`, postal_code FROM `tblclinic` ORDER BY `clinic_id` DESC";
					...
					?>
						<tr>
							...
							<td style="text-align:center;">
								<button type="button" class="btn btn-xs btn-warning" data-toggle="modal" data-target="#modalEdit<?php echo strtoupper($row['clinic_id'])?>"><small><b><i class="fa fa-pencil"></i> </b></small></button>
								<button type="button" class="btn btn-xs btn-danger" data-toggle="modal" data-target="#modalRemove<?php echo strtoupper($row['clinic_id'])?>"><small><b><i class="fa fa-times"></i> </b></small></button>
							</td>
						</tr>
